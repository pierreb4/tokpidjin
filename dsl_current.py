...
try:
    ...
    # Use stderr to avoid polluting stdout, and only show once
    if os.environ.get('EXPECT_GPU', '1') != '0' and not hasattr(sys, '_dsl_cupy_warning_shown'):
        print("CuPy not available, using CPU only", file=sys.stderr)
        sys._dsl_cupy_warning_shown = True
...
def _batch_grid_operations(grids, operation):
    """
...
def _gpu_grid_transform(grid, transform_func):
    """
...
# Pre-computed neighbor offsets for efficient neighbor computation in objects() and objects_t()
_DNEIGHBOR_OFFSETS = ((-1, 0), (1, 0), (0, -1), (0, 1))  # 4-connected neighbors (orthogonal)
_INEIGHBOR_OFFSETS = ((-1, -1), (-1, 1), (1, -1), (1, 1))  # 8-connected (diagonal) neighbors only
...
def identity(
    ...
    """ identity function """
    # logger.info(f'identity: {x = }')  # Disabled: excessive logging overhead
    return x


def first(container: 'Container') -> 'Any':
    """First item of container"""
    # logger.info(f'first: {container = }')  # Disabled: excessive logging overhead
...
def second(container: 'Container') -> 'Any':
    ...
    # logger.info(f'second: {container = }')  # Disabled: excessive logging overhead
    iterator = iter(container)
    next(iterator)
    return next(iterator, None)
...
def difference_tuple(a: 'Tuple', b: 'Tuple') -> 'Tuple':
    """Tuple difference"""
    # logger.info(f'difference_tuple: {a = }, {b = }')  # Disabled: excessive logging overhead
...
def dedupe_pair_tuple(S: 'Samples') -> 'Samples':
    ...
    # logger.info(f'dedupe_pair_tuple: {S = }')  # Disabled: excessive logging overhead
...
def c_iz(S: 'Samples', function: 'Callable') -> 'Any':
    ...
    x1 = apply(first, S)
    x2 = apply(second, S)
    x3 = dedupe(mapply(function, x1))
    x4 = dedupe(mapply(function, x2))
    return difference_tuple(x3, x4)
...
# Alias for compatibility with solvers
get_nth_t = get_nth
get_nth_f = get_nth
...
# Parameter table for o_g function - maps type codes to (by_color, by_count, by_position)
_O_G_PARAMS = [
...
def merge(
    ...
    # Check if merging frozensets - if so, return frozenset
...
# ============================================================================
# Compatibility Aliases for Solver Functions
# ============================================================================
# These aliases provide backwards compatibility with solvers that use the
# _t (tuple) and _f (frozenset) suffixed versions of generic functions.
# After consolidation, we keep only the Container-generic versions but provide
# aliases for the specialized variants used in solvers.

# Size and count functions
# ===== FrozenSet-specific (_f) function variants =====
# These are separate implementations that work with frozensets (Objects, Patches)
# They cannot be simple aliases because the implementations differ fundamentally

def p_f( element: 'FrozenSet' ) -> 'IntegerSet':
    """ colors occurring in object """
    return frozenset(c for _, _, c in element)

def palette_f( element: 'FrozenSet' ) -> 'IntegerSet':
    """ Extract colors from object element (frozenset variant) """
    return frozenset(c for _, _, c in element)

def mir_rot_f( patch: 'Patch', type: 'A4' ) -> 'Patch':
    """ Mirror/rotate frozenset patch """
    if type == 0:
        # Horizontal mirror
        return hmirror_f(patch)
    elif type == 1:
...
def get_nth_by_key_f( container: 'FrozenSet', rank: 'FL', key=identity ) -> 'Any':
    """Nth item of container, 0-based, using key function"""
...
def mir_rot(patch: 'Patch', type: 'A4' ) -> 'Patch':
    """ Alias for mir_rot_f - mirror/rotate patch """
    return mir_rot_f(patch, type)

# Aliases for _t suffixed functions (generic container versions)
size_t = size
colorcount_t = colorcount
...
# Ranking functions (generic Container versions)
get_arg_rank_t = get_arg_rank
get_common_rank_t = get_common_rank

# Shape and portrait (generic Container versions)
portrait_t = portrait
shape_t = shape

# Color functions (used internally by objects())
mostcolor_t = mostcolor
leastcolor_t = leastcolor
